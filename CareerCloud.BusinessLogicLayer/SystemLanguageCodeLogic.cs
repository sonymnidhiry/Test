        ...
        protected virtual void Verify(SystemLanguageCodePoco[] pocos)
        {
            ...
            foreach (SystemLanguageCodePoco poco in pocos)
            {
                if (string.IsNullOrEmpty(poco.LanguageID))
                    exceptions.Add(new ValidationException(1000, $"Critical Error occured! \n *LanguageID* field cannot be blank"));
                if (string.IsNullOrEmpty(poco.Name))
                    exceptions.Add(new ValidationException(1001, $"Critical Error occured! \n *Name* field cannot be blank"));
                if (string.IsNullOrEmpty(poco.NativeName))
                    exceptions.Add(new ValidationException(1002, $"Critical Error occured! \n *Native Name* field cannot be blank"));
